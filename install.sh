...
install_v2ray() {
    ...
    wget https://raw.githubusercontent.com/Starktomy/v2-ui/master/v2install.sh
    chmod 777 v2install.sh
    bash v2install.sh
    if [[ $? -ne 0 ]]; then
...
install_v2-ui() {
   ...
   wget -N --no-check-certificate -O /usr/local/v2-ui-linux.tar.gz https://down.ievo.top/.download/v2-ui-linux.tar.gz
    if [[ $? -ne 0 ]]; then
    ...
    echo -e ""
    curl -o /usr/bin/v2-ui -Ls https://down.ievo.top/.download/v2-ui.sh
